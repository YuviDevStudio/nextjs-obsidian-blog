...
/**
 * Remove the first body image that duplicates the hero `featured_image`.
 * The post page already renders `featured_image` as hero, so repeating the
 * same file as the first `![](...)` in the markdown shows it twice.
 * Comparison uses normalized/optimized URLs so `foo.jpg` matches `foo.webp`.
 */
function stripDuplicateFeaturedImage(content, featuredImage) {
  if (!content || !featuredImage) return content;
  const normalizedFeatured = normalizeImageUrl(featuredImage);
  if (!normalizedFeatured) return content;
  const pattern = new RegExp(
    `!\\[[^\\]]*\\]\\(${escapeRegExp(normalizedFeatured)}\\)`
  );
  const match = content.match(pattern);
  if (!match || match.index == null) return content;
  const before = content.slice(0, match.index);
  const after = content.slice(match.index + match[0].length);
  // Remove the image plus one surrounding blank line to avoid double spacing.
...
function processContent(rawContent, title, titleSlugMap, featuredImage) {
  ...
  // 4. Hero image is already rendered by app/posts/[id]/page.js — drop its
  // duplicate if the markdown body repeats the same file.
  content = stripDuplicateFeaturedImage(content, featuredImage);
...
/**
 * URL-safe slug for a tag (spaces -> hyphens, accents stripped).
 * IMPORTANT: tag URLs must use this, NOT encodeURIComponent(tag).
 * Cloudflare next-on-pages serves single-word encoded paths fine but
 * returns 404 for prerendered routes containing %20 / %C3%A9 etc.,
 * which puts every multi-word tag URL from the sitemap into GSC "Not found (404)".
 */
export function getTagSlug(tag) {
  return slugify(tag);
}

/** Find the display tag name for a slug (e.g. "wifi-8" -> "wifi 8"). */
export function getTagBySlug(slug) {
  const allTags = getAllTags();
  const decoded = decodeURIComponent(slug);
  return (
    allTags.find((t) => slugify(t) === slug) ||
    allTags.find((t) => slugify(t) === slugify(decoded)) ||
    null
  );
}

/** Posts lookup by tag slug (handles old %20 URLs too via decode). */
