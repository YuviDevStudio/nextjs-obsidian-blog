    ...
    <div className="w-full">
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-4 px-2">
          <h2 className="text-lg md:text-xl font-bold font-display uppercase tracking-wider text-slate-400 dark:text-slate-500">
            Destacados
          </h2>
        </div>
              ...
              {allTags.map(tag => {
                ...
                return (
                  <Link
                    key={tag}
                    href={`/tags/${tag}`}
                    className="px-2.5 py-1 text-xs rounded-lg border border-slate-200/60 dark:border-slate-800 bg-white dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 hover:text-indigo-600 dark:hover:text-sky-400 hover:border-indigo-200 dark:hover:border-sky-500/30 transition-all font-medium"
                  >
                    #{capitalized}
                  </Link>
                );
